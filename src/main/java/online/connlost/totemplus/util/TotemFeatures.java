...
public class TotemFeatures {
    ...
    public static Vec3d findSafePos(Entity player) {
        ...
        for (int i = blockPoses.size() - 1; i >= 0; i--) {
            if (player.getWorld().getBlockState(blockPoses.get(i).down()).isAir()) continue;
            return new Vec3d(blockPoses.get(i).getX(), blockPoses.get(i).getY(), blockPoses.get(i).getZ());
        }

        // fallback
        if (!player.getWorld().getBlockState(new BlockPos(0, 64, 0)).isAir()) {
            for (int i = 65; i < 256; i++) {
                if (!player.getWorld().getBlockState(new BlockPos(0, i, 0)).isAir()) continue;
                return new Vec3d(0, i, 0);
            }
        }
        if (player.getWorld().getBlockState(new BlockPos(0, 63, 0)).isAir()) {
            for (int i = 62; i > 0; i--) {
                if (player.getWorld().getBlockState(new BlockPos(0, i, 0)).isAir()) continue;
